...
RSpec.describe CommonSteps::QuotationConfirmedController, type: :controller do
	describe "GET new" do
  ...
  describe 'POST confirm_quotation' do
    ...
    it "confirms quotation succesfully" do
    	sign_in(user)
    	allow(CommonSteps::QuotationConfirmed).to receive(:confirm_quotation).and_return('success')
	  ...
	  it "confirms quotation fails" do
	  	sign_in(user)
	  	allow(CommonSteps::QuotationConfirmed).to receive(:confirm_quotation).and_return('fail')
  ...
  describe 'POST update_pricing' do
    ...
    it "confirms quotation by pricing succesfully" do
    	sign_in(user)
    	CommonSteps::QuotationConfirmed.create(operation_id: operation.id)
      post :update_pricing, params: {secure_id: operation.secure_id}
      expect(flash[:notice]).to eq("Quotation correctly sent")   
      expect(response).to redirect_to common_steps_new_quotation_view_path(operation.secure_id)
  	end
  end
end
