class LclSteps::CargoInfo < ApplicationRecord
	  self.table_name = 'lcl_steps_cargo_infos'
	  ...
	  def self.create_cargo_info(params, fcl_cargo_info_params)
	  	...
	  	existing_cargo_info = LclSteps::CargoInfo.find_by(operation_id: op)
	    if existing_cargo_info.created_at == existing_cargo_info.updated_at
	      op.update(status: 'IN PROGRESS', current_step: op.current_step + 1, status_message: 'Request Booking Order')
	    end
	  ...
	  private
		  def self.create_pieces (params_array, cargo_info)
		  	CommonSteps::Piece.where(lcl_steps_cargo_info_id: cargo_info.id).delete_all
		  	params_array = params_array.drop(3)[0..-4]
	      (0..params_array.length).step(9) do |element|
	      	unless params_array[element].nil?
	        	piece = CommonSteps::Piece.create(lcl_steps_cargo_info_id: cargo_info.id, gross_weight: params_array[element][1], length: params_array[element + 1][1], width: params_array[element + 2][1], height: params_array[element + 3][1] ,commercial_description: params_array[element+4][1], container_size: params_array[element+5][1], cargo_hazardous: params_array[element+6][1], hazardous_class: params_array[element+7][1], un_code: params_array[element+8][1] )
	      	end
	      end
	    end
	  	...
	  	def self.create_bonded_task(params, op)
	  		...
	  		if params[:lcl_steps_cargo_info][:bonded] == 'true'
	  	...
	  	def self.create_self_propelled_task(params, op)
	  		...
	  		if params[:lcl_steps_cargo_info][:self_propelled] == 'true'
	  	...
	  	def self.update_shipper_ein(params, op)
	  		...
	  		if shipper.ein != params[:lcl_steps_cargo_info][:ein]
	        shipper.update_attribute('ein', params[:lcl_steps_cargo_info][:ein])
	  		end
	  	end
end
