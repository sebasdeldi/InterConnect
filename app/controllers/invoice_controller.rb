class InvoiceController < ApplicationController
	...
	def new
	  ...
	  existing = Invoice.find_by(operation_id: Operation.find_by(secure_id: params[:secure_id]).id)
	  if existing
	    @invoice = existing
	  else
	    @invoice = Sli.new
	...
	def create
	  if Invoice.find_by(operation_id: Operation.find_by(secure_id: params[:secure_id]).id)
	    if Invoice.update(invoice_params.merge(requested: true))
	      redirect(Operation.find_by(secure_id: params[:secure_id]).id)
	    end
	  else
	    if Invoice.create(invoice_params.merge(requested: true))
	      redirect(Operation.find_by(secure_id: params[:secure_id]).id)
	    end
	  end
	end
