  ...
  class InsuranceController < ApplicationController
    ...
    def view_pdf
      ...
      respond_to do |format|
        format.html
        format.pdf do
          render template: "documents/insurance/view_pdf", pdf: "insurance_#{@operation.reference}"   # Excluding ".pdf" extension.
        end
      end
    end
    ...
    def send_info
      ...
      FclExwOperationMailer.send_insurance_info(request.base_url+documents_insurance_form_path(operation.secure_id), operation, current_user).deliver_later
