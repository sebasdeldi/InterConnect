module FclExwSteps	
	class FclExwCargoInfoStepsController < ApplicationController
		def new
			@fcl_cargo_info = FclExwSteps::FclExwCargoInfoStep.new
		...
		def create
		  ...
		  if FclExwSteps::FclExwCargoInfoStep.create_cargo_info(params, fcl_cargo_info_params)
		...
		private
			def fcl_cargo_info_params
				params.require(:fcl_exw_steps_fcl_exw_cargo_info_step).permit({files: []}, :operation_id, :loading_address, :loading_date, :loading_time, :gross_weight, :commercial_description, :cargo_hazardous, :hazardous_document, :pickup_reference, :contact_name, :contact_email, :contact_number, :contact_company, :pieces_number, :bonded, :self_propelled)
			end
	end
end
